...
print_usage() {
    ...
    echo "  $0 run --help"
    echo "  $0 run --oscar-endpoint https://oscar.example.com --oscar-token TOKEN workflow.cwl input.json"
    echo "  $0 test"
}
...
test_image() {
    ...
    if [[ "$ERROR_OUTPUT" == *"either --oscar-token or --oscar-username is required"* ]]; then
...
show_examples() {
    ...
    echo "   $0 run --oscar-endpoint https://oscar.example.com \\"
    echo "          --oscar-token YOUR_TOKEN \\"
    echo "          workflow.cwl input.json"
    echo ""
    echo -e "${YELLOW}2. Run with username/password authentication:${NC}"
    echo "   $0 run --oscar-endpoint https://oscar.example.com \\"
    echo "          --oscar-username your_user \\"
    echo "          --oscar-password your_pass \\"
    echo "          workflow.cwl input.json"
    echo ""
    ...
    echo "       $DOCKER_IMAGE \\"
    echo "       --oscar-endpoint https://oscar.example.com \\"
    echo "       --oscar-token YOUR_TOKEN \\"
    echo "       /workspace/workflow.cwl /workspace/input.json"
    echo ""
    echo -e "${YELLOW}6. Use with environment variables:${NC}"
    echo "   export OSCAR_ENDPOINT=https://oscar.example.com"
    echo "   export OSCAR_TOKEN=your_token_here"
    echo "   docker run --rm -v \$(pwd):/workspace -w /workspace \\"
    echo "       -e OSCAR_ENDPOINT -e OSCAR_TOKEN \\"
